...
getRetrosheet <- function(type, team, schedSplit = NULL, stringsAsFactors = FALSE, cache = NA) {
   ...
   # fname <- unzip(tmp, list = TRUE)$Name
    ...
    if (type == "schedule") {
        ...
        zcon <- "./schedules/2026schedule.csv"
    ...
    zcon <- unz(tmp, filename = paste0("TEAM"))
